...
The `GHlmdi` object contains everything needed to graph the results. 
But first, we need to expand the data from matrices to tidy format and
select the columns we want.
The focus here is on the aggregate (`D_agg_cum`) and disaggregate (`D_cum`)
factors that affect changes in useful exergy delivered to the economy.

```{r}
GHlmdi_tidy <- GHlmdi %>% 
  select(Country, Year, D_agg_cum, D_cum) %>% 
  gather(key = varname, value = value, -Country, -Year) %>% 
  expand_to_tidy(matnames = "varname", matvals = "value", 
                 rownames = "rowname", colnames = "colname", 
                 rowtypes = "rowtype", coltypes = "coltype") %>% 
  select(-colname, -rowtype, -coltype)
head(GHlmdi_tidy, 10)
```

Next, we map the values of `varname` and `rowname` 
to variable names appropriate for graphing.
We also set the levels of the Factors,
and therefore the order they will appear in the legend of the graph.
We choose an order that mimics the vertical position of the data in the graph.

```{r}
GHlmdi_forgraphing <- GHlmdi_tidy %>% 
  mutate(
    Factor = case_when(
      varname == "D_agg_cum" ~ "D_tot", 
      rowname == "EXp.ktoe" ~ "D_ex",
      rowname == "eta_ij" ~ "D_eff", 
      rowname == "phi_i"  ~ "D_str", 
      rowname == "phi_ij" ~ "D_dil" 
    ), 
    Factor = factor(Factor, levels = c("D_tot", "D_ex", "D_str", "D_eff", "D_dil"))
  ) %>% 
  select(-rowname, -varname) %>% 
  group_by(Country, Factor)
head(GHlmdi_forgraphing, 10)
```

Finally, we create a graph showing both the total changes and the factors 
that comprise the total change.
...
[Heun and Brockway [-@Heun:2019]](https://doi.org/xxxx).


## Conclusion

The `LMDIR` package provides a very convenient and powerful approach to log-mean divisia index
analysis of energy conversion chains.
