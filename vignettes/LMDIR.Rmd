...
fillrowX <- matrix(c(1, 1, 0, 1), byrow = TRUE, nrow = 1, ncol = 4,
                   dimnames = list("HTH.600.C - Electric heaters", 
                                   c("EXp.ktoe", "phi_i", "phi_ij", "eta_ij"))) %>% 
  sort_rows_cols()
GHlmdi <- XGH %>% 
  group_by(Country) %>% 
  lmdi(fillrow = fillrowX)
glimpse(GHlmdi)
```
...
### A note about special cases when calculating entires in $\mat{Z}$
...
Finally, we create a graph showing both the total changes ($D_{tot}$) and the factors 
