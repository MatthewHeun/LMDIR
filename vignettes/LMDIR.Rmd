...
The format of the $\mat{X}$ matrices is shown below.
...
(Specifically, 0.6225 + .0204 + 0.1314 + .2258 = 1.)
Within each subsubcategory, $\phi_{ij}$ values sum to 1.
(E.g., for Mechanical drive fueled by food and feed, 0.8416 + 0.1584 = 1)
...
To perform LMDI analysis, we use the `lmdi()` function after grouping on metadata variables
